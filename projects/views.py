...
def projects(request):
    ...
    for project in projects:
        ...
        for item in tasks:
            project.total_budget += item.budget
    ...
    if request.user == project.user.user:
        ...
        if request.method == 'POST' and 'offer_submit' in request.POST:
            ...
            if task_offer_form.is_valid():
                ...
                task_offer.task = Task.objects.get(
        ...
        if request.method == 'POST':
            ...
            if task_file_form.is_valid():
                ...
                existing_file = task.files.filter(
                    file=directory_path(
                        task_file, task_file.file.file)).first()
                ...
                if (access):
                    ...
                    if request.user.profile != project.user and request.user.profile != accepted_task_offer.offerer:
                        teams = request.user.profile.teams.filter(
                            task__id=task.id)
                        for team in teams:
                            tft = TaskFileTeam()
                            tft.team = team
...
def task_view(request, project_id, task_id):
    ...
    user_permissions = get_user_task_permissions(request.user, task)
    if not user_permissions['read'] and not user_permissions['write'] and not user_permissions[
            'modify'] and not user_permissions['owner'] and not user_permissions['view_task']:
    ...
    if user_permissions['read'] or user_permissions['write'] or user_permissions[
            'modify'] or user_permissions['owner'] or user_permissions['view_task']:
